...
function selecionarProduto() {
    ...
    $.getJSON("http://localhost:3000/pecas/json/" + selecionado, function (data) {

        $("#adicionar-descricao").val(data.nome);
        $("#adicionar-valor").val(data.valor_unitario);
...
function buscarPecas(){
	jQuery.ajax({
			type: "GET",
			url: "http://localhost:3000/pecas/json",
			mimeType: "multipart/form-data",
			dataType: "json",
			success: function (data) {
...
function adicionarProduto(data){
    ...
    $.each(data, function (key, val) {
        linha+="<option id='" + val._id + "' >"+ val.nome + "</option>";
    });
...
function buscarProduto(idLinha){
    ...
    jQuery.ajax({
            type: "GET",
            url: "http://localhost:3000/pecas/json/"+idProduto,
            mimeType: "multipart/form-data",
            dataType: "json",
            success: function (data) {
                carregarProduto(idLinha, data);
            }
        });
}
//Método que coloca nos inputs de descricao e o valor, os dados do produto.
function carregarProduto(idLinha, produto){
    $("#adicionar-descricao-"+idLinha).val(produto.nome);
    $("#adicionar-valor-"+idLinha).val(produto.valor_unitario);
